...
import { chatSummary, leaveChat } from "@/lib/linq-sandbox";
import { sandboxHandler, tenantFrom } from "@/lib/linq-sandbox-api";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

/** POST /v3/chats/{chatId}/leave - leave a group chat (422 for 1:1 chats). */
export const POST = sandboxHandler((req, chatId) => {
  const tenant = tenantFrom(req);
  const chat = leaveChat(tenant.id, chatId);
